...
export async function pushNotificationFormAction(_prevState: ActionState, formData: FormData) {
  ...
  if (!isUpdate) {
    return await fromErrorToActionState(new Error('Use the dedicated create page for new notifications'));
  }
  ...
  redirect(`/dashboard/push_notifications?success=${encodeURIComponent(await actionMessage('pushNotificationUpdated'))}`);
}
