...
describe('Stock DeleteModal', () => {
  ...
  describe('Modal Content', () => {
    ...
    it('renders modal content when open', () => {
      expect(screen.getByText('Delete Stock Record')).toBeInTheDocument();
      expect(screen.getByText(/Are you sure you want to delete stock record for/)).toBeInTheDocument();
      expect(screen.getByText('Product A - Branch B')).toBeInTheDocument();
  ...
  describe('Modal Actions', () => {
    ...
    it('calls deleteStock when delete button is clicked', async () => {
      ...
      expect(deleteStock).toHaveBeenCalledWith('1');
    });
  });
  ...
  describe('Delete Success', () => {
    it('shows success toast and refreshes router on successful delete', async () => {
      ...
      await waitFor(() => {
        expect(toast.success).toHaveBeenCalledWith('Stock record deleted successfully');
  ...
  describe('Delete Error', () => {
    it('shows error toast when delete returns error', async () => {
      ...
      await waitFor(() => {
        expect(toast.error).toHaveBeenCalledWith('Failed to delete stock record');
      });
    });
  ...
  describe('Loading State', () => {
    it('shows loading text and disables buttons when deleting', async () => {
      // Mock deleteStock to not resolve immediately
      let resolveDelete: () => void;
      const deletePromise = new Promise<{ error: null }>((resolve) => {
        resolveDelete = () => resolve({ error: null });
      });
      (deleteStock as jest.Mock).mockReturnValue(deletePromise);
  ...
  describe('Props', () => {
    it('uses stockId for delete operation', async () => {
      ...
      expect(deleteStock).toHaveBeenCalledWith('test-stock-123');
    });
    ...
    it('displays stockDescription in confirmation message', async () => {
      ...
      expect(screen.getByText('Special Product - Main Branch')).toBeInTheDocument();
    });
  });
});