...
describe('ToastHandler', () => {
  ...
  const setSearch = (search: string) => {
    window.history.replaceState(null, '', `/dashboard/plan_limits${search}`);
  };
  ...
  it('shows toast.success when success param is present', () => {
    setSearch('?success=Plan+limit+created');
    replaceStateSpy.mockClear();

    render(<ToastHandler />);

    expect(toast.success).toHaveBeenCalledWith('Plan limit created');
  });
