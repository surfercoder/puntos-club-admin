...
describe('ToastHandler (push_notifications_crud)', () => {
  let replaceStateSpy: jest.SpyInstance;

  const setSearch = (search: string) => {
    window.history.replaceState(null, '', `/dashboard/push_notifications${search}`);
  };
  ...
  it('shows toast.success when success param is present', () => {
    setSearch('?success=Notification+created');
    replaceStateSpy.mockClear();

    render(<ToastHandler />);

    expect(toast.success).toHaveBeenCalledWith('Notification created');
  });
